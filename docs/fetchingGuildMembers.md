...
As you can see, the "no overlap" method can be a lot quicker than the "overlap" method, however, it's also a lot less effective. After doing a few tests with both methods ("overlap" and "no overlap"), "no overlap" shows a lot less consistency/reliability than "overlap".
